...
# (with anonymisation) for sample companies.
# ---------------------------------------------------------------------------
_BASE_QA_ENTRIES: list[dict] = [
    # 이주정산 - 8 entries
    {
        "category": "이주정산",
    ...
    },
    {
        ...
        "answer": "퇴거일에 관리사무소 또는 해당 공급기관에서 계량기를 검침합니다. 수도: 관리사무소에서 검침 후 일할 정산. 전기: 한전에 이사정산 신청(☎0XX-XXXX-XXXX) 또는 관리사무소 통해 정산. 도시가스: 도시가스 회사에 이사정산 신청, 퇴거 당일 밸브 차단. 각 공과금은 최종 검침일 기준으로 계산되어 보증금에서 공제됩니다.",
...
# ---------------------------------------------------------------------------
SAMPLE_COMPANY_CONFIGS: list[dict] = [
    {
        # --- Company ---
        "company_id": 1000,
        "company_name": "샘플오피스텔",
    ...
    },
    {
        ...
        "categories": [
            ...
            {"label": "주방", "question": "씽크대 물이 안나와요."},
            {"label": "화장실", "question": "변기가 고장났어요."},
            {"label": "중간관리비 정산", "question": "이주정산 절차가 궁금해요."},
            {"label": "승강기", "question": "엘리베이터가 고장났어요."},
            {"label": "가전/AS", "question": "TV가 안나와요."},
            {"label": "주차장", "question": "차량 등록은 어떻게 하나요?"},
            {"label": "급배수/설비", "question": "싱크대 하수구에서 냄새가 나요."},
            {"label": "소음", "question": "층간소음 방송 부탁드려요."},
            {"label": "방범/보안", "question": "월패드 침입경보가 계속 울려요."},
            {"label": "관리비", "question": "관리비 납부 방법이 궁금해요."},
            {"label": "각종 시설물 AS", "question": "등 교체는 어떻게 하나요?"},
            {"label": "소방", "question": "가스경보기가 오작동해요."},
            {"label": "청소/미화", "question": "흡연 금지 방송 부탁드려요."},
            {"label": "분리수거", "question": "대형 폐기물은 어떻게 버리나요?"},
            {"label": "입주신고", "question": "입주신고 시 필요한 서류가 궁금해요."},
        ],
        # --- Tenant quota (enterprise) ---
        "quota": {
            "monthly_chat_cnt": 500,
...
def seed_sample_companies(db: Session) -> None:
    ...
    for cfg in SAMPLE_COMPANY_CONFIGS:
        ...
        if db.query(TenantQuota).filter(TenantQuota.company_id == cid).count() == 0:
            ...
            quota = TenantQuota(
                company_id=cid,
                monthly_chat_cnt=quota_cfg.get("monthly_chat_cnt", 500),
                monthly_tokens=quota_cfg.get("monthly_tokens", 200000),
                monthly_embed_cnt=quota_cfg.get("monthly_embed_cnt", 1000),
            )
            db.add(quota)
            db.commit()
        ...
        if admin_cfg:
            exists = (
                db.query(AdminUser)
                .filter(
                    AdminUser.company_id == cid,
                    AdminUser.username == admin_cfg["username"],
                )
                .count()
            )
            if exists == 0:
...
def seed_data(db: Session):
    ...
    if db.query(QaKnowledge).filter(QaKnowledge.company_id == 1).count() == 0:
        qa_entries = copy.deepcopy(_BASE_QA_ENTRIES)
        # company_id=1 keeps the original phone numbers (☎123 etc.)
        # Restore the original answer for index 5 (with ☎123)
        qa_entries[5]["answer"] = (
            "퇴거일에 관리사무소 또는 해당 공급기관에서 계량기를 검침합니다. "
            "수도: 관리사무소에서 검침 후 일할 정산. "
            "전기: 한전에 이사정산 신청(☎123) 또는 관리사무소 통해 정산. "
    ...
    # --- Seed sample companies (company_id 1000+) ---
    seed_sample_companies(db)
